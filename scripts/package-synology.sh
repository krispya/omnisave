...
mkdir -p "${ROOT}/dist"
# The omnisave-server prefix separates the spk from the client archives; the
# build number lives in INFO, which is what DSM actually reads.
OUTPUT="${ROOT}/dist/omnisave-server-${VERSION}-${SPK_ARCH}.spk"
