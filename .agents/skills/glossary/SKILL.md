---
name: "glossary"
description: "Maintain an app's glossary of terms in docs/GLOSSARY.md. Look up a term, add a new one, or audit the glossary against the codebase, FDRs, and ADRs for missing or stale entries."
---

# Glossary

The canonical vocabulary for the app. Lives at `docs/GLOSSARY.md`, organised into four sections — UI, Product, Authorization, Backend — siblings of [FDRs](../../../docs/fdr/INDEX.md) and [ADRs](../../../docs/adr/INDEX.md).
...
4. **Backend** — infrastructure jargon. If only contributors say the word, it goes here (NATS, JetStream, Stream, KV, Subject, Live Event, OCC, Nanoid).
...
- **Authorization** opens with the permission model's primitives (the concepts everything else is defined in terms of), then the named/built-in roles, then edge concepts (scopes, overrides, special-case boundaries).
- **Product** opens with the structural nouns (the containers users navigate, largest to smallest), then the content that lives inside them, then ambient features that accompany the content.
- **UI** opens with the top-level layout regions in reading order (left to right, outside in), then the elements within them, then secondary surfaces (overlays, pickers, transient panels).
- **Backend** opens with the system's outermost boundary (the API/core seam), then the storage and transport substrate, then concepts derived from those primitives.
...
- Standard tech terms with no app-specific meaning ("PostgreSQL", "WebSocket").
