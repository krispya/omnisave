...
// A game the adapter already found save content for keeps that one
// representation: profile rules that would rediscover the same progress
// under another layout are not consulted, so one game never tracks two
// saves whose lineages could never converge (FDR-003, decision 10).
