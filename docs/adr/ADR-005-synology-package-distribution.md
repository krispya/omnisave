...
while respecting DSM's package lifecycle and persistent-directory conventions.
...
volume-specific paths those resolve to, so the package remains movable between
volumes.
...
- SPK metadata, lifecycle scripts, and separate architecture packages must stay
  tested alongside the server.
- Uninstalling does not reclaim the space the save history occupies; operators
  who want it gone must remove the package `var` directory themselves.
- The package cannot run on DSM releases older than 7.0-40314.
- SPKs require installation testing on representative DSM hardware even when
  their archive structure validates in CI.
