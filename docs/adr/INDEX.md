...
Architecture Decision Records capture Omnisave's cross-cutting technical decisions and the context that produced them. Feature-specific decisions live in [FDRs](../fdr/INDEX.md), which cite ADRs; citations flow FDR → ADR only.

**Each record describes the decision as it stands today.** A decision that changes is rewritten, and one that stops applying is deleted — not amended with a note, and not left behind as a tombstone with a supersession chain. Git holds what a record used to say; this directory holds what is true. A reader who opens one of these should not have to reconcile it against a later one to learn what the system does.
