...
**Omnisave**. One independently named and synchronized game save on the server. It owns a revision tree and one global _Current Revision_. Restoring moves that pointer. Forking creates another omnisave when progress must synchronize independently. In everyday use it is just "save." An adapter-native save on disk is always a _Local Save_. An omnisave always carries a display name. The server assigns one when creation omits it ("Save N," while forks inherit the source name plus " (fork)"), and numbers a requested name the game's saves already carry ("Steam Deck 2"). A name can be changed but never cleared (see [FDR-003](fdr/FDR-003-automatic-save-binding.md) and [FDR-005](fdr/FDR-005-save-sync.md)).
