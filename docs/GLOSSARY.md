...
**Tracking** — This machine's selection of which discovered games to synchronize, chosen in the `track` prompt and persisted in local tracking state (`client.json` in the user config directory). Games that disappear from a scan stay tracked until explicitly unselected.
