...
- A restore into such a game is therefore complete only when the store's registry matches the applied revision, and that reconciliation step is not built: applying a revision must also register restored files the store no longer lists, and whether registry entries the revision lacks must be removed is still unmeasured. The settled direction for several bound Devices remains a server-granted placement claim, with the claim holder's content entering the store once rather than each Device arguing with the cloud on its own.
