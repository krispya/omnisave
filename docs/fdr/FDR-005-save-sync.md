...
- A lineage minted under the retired mirror vocabulary migrates to the game's own save location by a proven in-place rename the first time a pass works its history (decision 14). Until a device proves the mapping, the lineage is reported as held rather than left looking restorable.
...
### 14. Mirror lineages migrate by proven rename, when their history is touched

**Decision:** A lineage whose revisions speak the retired mirror vocabulary is migrated by renaming every path in place — `remote/rest` becomes the rule location and prefix this Device's own save demonstrates — leaving revision identities, artifacts, ancestry, bindings, and achievements untouched. The mapping is proven, never guessed: each mirror name matching exactly one path of the local save's manifest nominates an anchoring, every nomination must agree, at least one is required, and hash agreement is counted as corroboration. The server refuses any rename that would not be total — a fork family, a mixed vocabulary, a lineage with nothing to rename. The attempt rides on histories a pass loads anyway, so a settled save still costs nothing; an unproven lineage is reported as held every time its history is worked. **Why:** History minted from the mirror cannot bind, verify, or restore into the folder the game reads (FDR-003, decision 10), and a rename is the only migration that keeps every reference into the lineage valid and stays mechanically reversible. Evidence is local by nature — only a device holding the native save can know where the mirror's names land — which is also why migration converges per device rather than running once centrally. **Tradeoff:** Convergence is not finality: a lineage no device can prove waits indefinitely, visibly. A game whose per-OS rules anchor at different depths migrates into the vocabulary of whichever OS proves it first, and other OSes still cannot adopt it (decision 11's limit). Files the mirror captured that the game had deregistered are indistinguishable in hindsight and stay in history; restoring them may resurrect an abandoned run — visible in the game, and re-abandonable — which is the safe side of the asymmetry against silently pruning live runs.
