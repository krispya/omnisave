...
- When Steam Cloud carries a game's save between Devices, every bound Device follows the same mirror Steam itself replicates, and a restore should enter that transport exactly once. The settled direction is a server-granted placement claim: one Device wins the claim and writes its mirror, Steam fans the content out, and every other Device reports the wait and adopts the arriving content through the existing exact-match rebind. Not built yet — today each bound Device still applies a pull to its own mirror independently, which can make Steam see the same change as several local edits.
